...
import { OrdinamentoNormale, useMutuo } from "@/Context/MutuoContext";
import React, { useEffect } from "react";


export interface FiltroItem {
  value: OrdinamentoNormale;
  label: string;
  icon: string;
  description: string;
}
...
export interface FiltroOrdinamentoProps {
  handleFiltro: (nomeFiltro: OrdinamentoNormale) => void;
}

export default function FiltroNormale({ handleFiltro }: FiltroOrdinamentoProps) {
  const { ordinamentoNormale, setOrdinamentoNormale } = useMutuo();
  ...
  const handleClick = (tipo: OrdinamentoNormale) => {
    if (ordinamentoNormale !== tipo) {
      setOrdinamentoNormale(tipo);
      handleFiltro(tipo);
    }
  };

  const filtroSelezionato = filtri.find(f => f.value === ordinamentoNormale);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {filtri.map((filtro) => {
          const isSelected = ordinamentoNormale === filtro.value;
