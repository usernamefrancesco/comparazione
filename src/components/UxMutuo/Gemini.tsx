...
export default function GeminiMutuo() {
  ...
  const ScoreBadge = ({ score }: { score: number }) => {
    ...
    return (
      <div className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${getScoreStyle(score)}`}>
  ...
  const Badge = ({ children, variant = "default" }: { children: React.ReactNode; variant?: "default" | "green" | "blue" | "purple" }) => {
    const variants = {
  ...
  const DetailItem = ({ label, value, highlight = false }: { label: string; value: React.ReactNode; highlight?: boolean }) => (
    <div>
      <div className={`text-xl font-semibold ${highlight ? 'text-blue-600' : 'text-gray-900'}`}>
        {value}
      </div>
  ...
  const CostItem = ({ label, value }: { label: string; value: React.ReactNode }) => (
  ...
  const RequirementItem = ({ title, icon: Icon, children }: { title: string; icon: React.ElementType; children: React.ReactNode }) => (
      ...
      <div className="max-w-3xl mx-auto py-10 px-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-start justify-between gap-4 bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="flex flex-col items-end gap-3">
              {selezionato.score !== undefined && <ScoreBadge score={selezionato.score} />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card title="Finanziamento" icon={Euro}>
            <DetailItem label="Importo mutuo" value={formatCurrency(selezionato.importoMutuo)} />
            <DetailItem label="Durata" value={`${selezionato.durataAnni} anni`} />
          ...
          <Card title="Tassi" icon={TrendingUp}>
            <DetailItem label="Tasso fisso" value={formatTasso(selezionato.tassoFisso)} />
            {selezionato.taeg && (
              <DetailItem label="TAEG" value={formatTasso(selezionato.taeg)} />
            )}
            <DetailItem label="Tipo tasso" value={selezionato.tipiTasso || "Fisso"} />
          </Card>

          <Card title="Pagamenti" icon={Calendar}>
            <DetailItem label="Rata mensile" value={formatCurrency(selezionato.rataMensile)} highlight />
            <DetailItem label="Rata totale" value={formatCurrency(selezionato.rataMensile + (selezionato.incassoRata?.importo || 0))} />
            {selezionato.totaleDaRimborsare && (
              <DetailItem label="Totale da rimborsare" value={formatCurrency(selezionato.totaleDaRimborsare)} />
            )}
          </Card>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-sm mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
            <CostItem 
              label="Spese perizia" 
              value={selezionato.spesePerizia?.importo > 0 
                ? formatCurrency(selezionato.spesePerizia.importo) 
                : "Gratuita"} 
            />
            <CostItem 
              label="Spese istruttoria" 
              value={selezionato.speseIstruttoria?.importo > 0 
                ? formatCurrency(selezionato.speseIstruttoria.importo) 
                : "Azzerata"} 
            />
            <CostItem 
              label="Imposta sostitutiva" 
              value={selezionato.impostaSostitutiva?.promozione 
                ? "Promozionale" 
                : formatCurrency(selezionato.impostaSostitutiva?.importo || 0)} 
            />
            <CostItem 
              label="Incasso rata" 
              value={selezionato.incassoRata?.esiste 
                ? formatCurrency(selezionato.incassoRata.importo) 
                : "Gratuito"} 
            />
            <CostItem 
              label="Assicurazioni" 
              value={`${formatCurrency(selezionato.assicurazioniObbligatorie?.costoStimato || 0)}/anno`} 
            />
          </div>
          
          {selezionato.totaliCostiExtra && (
            <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center">
              <span className="font-semibold text-gray-900">Totale costi accessori</span>
              <span className="font-bold text-lg text-gray-900">{formatCurrency(selezionato.totaliCostiExtra)}</span>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {selezionato.eta && (
              <RequirementItem title="Età" icon={Calendar}>
                <p>Da <span className="font-medium">{selezionato.eta.minima || 18} anni</span></p>
                <p>Fino a <span className="font-medium">{selezionato.eta.massima} anni</span></p>
                {selezionato.eta.maxUnder36 && (
                  <p className="text-blue-600 mt-2 font-medium text-xs">Vantaggi under 36</p>
                )}
              </RequirementItem>
            )}
            ...
            <RequirementItem title="Immobile" icon={Home}>
              {selezionato.soloClassiAB ? (
                <>
                  ...
                  <p className="text-xs text-gray-500">Sconti per alta efficienza</p>
                </>
              ) : (
                <p className="text-green-700 font-medium">Tutte le classi energetiche</p>
              )}
            </RequirementItem>
              ...
              <RequirementItem title="Assicurazioni" icon={Shield}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-gray-500">{selezionato.assicurazioniObbligatorie.assicurazioneCasa ? "Obbligatoria" : "Non obbligatoria"}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-gray-500">{selezionato.assicurazioniObbligatorie.assicurazioneVita ? "Non facoltativa" : "Facoltativa"}</span>
                  </div>
                </div>
              </RequirementItem>
            )}
        ...
        {((selezionato.proMutuo && selezionato.proMutuo.length > 0) || 
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 text-gray-700 mb-5">
              ...
              <h2 className="text-base font-semibold">Vantaggi e limitazioni</h2>
            </div>
              ...
              {selezionato.controMutuo && selezionato.controMutuo.length > 0 && (
                <div className="bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-semibold text-gray-800">Limitazioni</h3>
                  </div>
                  <ul className="space-y-3 text-sm text-gray-700 list-inside">
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-5 mb-8">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-base font-semibold text-blue-900 mb-2">Informazioni importanti</div>
              ...
              <p className="text-sm text-blue-800 font-medium">
                Assicurazioni: <span className="font-normal">Non sei obbligato a sottoscrivere le polizze con la banca che eroga il mutuo.</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center pt-6">
          <Button 
            className="bg-gray-900 hover:bg-gray-700 text-white px-8 py-3 rounded-lg font-semibold text-base shadow-lg transition-colors duration-200"
          >
            Richiedi Preventivo
          </Button>
